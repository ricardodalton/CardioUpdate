...
def level(item: dict) -> str:
    # Automated imports never assert Practice Changer before editorial review.
    ...
    try:
        ...
        for chunk in split_sentences(text):
            try:
                ...
                print(f"Traducción omitida para un fragmento: {exc}")
                out.append(chunk)
        return " ".join(out).strip()
    except Exception as exc:
        print(f"Traducción completa omitida: {exc}")
        return text
    ...
    for attempt in range(4):
        try:
            r = requests.get(EPMC_API, params=params, timeout=60,
                             headers={"User-Agent": "CardioUpdate/5.0 weekly evidence"})
            r.raise_for_status()
            return (r.json().get("resultList") or {}).get("result") or []
        except requests.RequestException as exc:
            if attempt == 3:
                print(f"Europe PMC: consulta omitida tras reintentos: {exc}")
                return []
...
def fetch_recent():
    ...
    groups = [
        ...
        '"congenital heart" OR fontan OR "tetralogy of fallot" OR coarctation OR "adult congenital"',
    ]
    results = {}
    for terms in groups:
...
def candidate_key(item: dict) -> str:
    ...
    if item.get("pmid"):
        return "pmid:" + norm(item["pmid"])
    title = norm(item.get("title", ""))
    return "title:" + title if title else ""


def paper_date(item: dict):
    raw = (item.get("firstPublicationDate") or item.get("date") or
           (item.get("journalInfo") or {}).get("printPublicationDate") or "")[:10]
    try:
...
def merge_candidate_pool(pool: list[dict], recent: list[dict]):
    ...
    for x in merged.values():
        ...
        if d is None or d >= cutoff:
            kept.append(x)
    kept.sort(key=lambda x: ((paper_date(x) or date.min).isoformat(), score(x)), reverse=True)
    return kept


def edition_window(day: date):
    # Friday issue covers Saturday...Friday (7 publication days).
    friday = day
    if friday.weekday() != 4:
...
def select_weekly(pool: list[dict], start: date, end: date):
    ...
    # High-quality core first. If that produces <20 papers, relax the score threshold,
    # but never import papers from outside the Saturday-Friday issue window.
    ...
    if len(chosen) < MIN_WEEKLY:
        ...
        for sc, x in eligible:
            if candidate_key(x) in chosen_keys or sc < 8:
                continue
            chosen.append(x)
            chosen_keys.add(candidate_key(x))
            if len(chosen) >= MIN_WEEKLY or len(chosen) >= MAX_WEEKLY:
                break
...
def crossref_verify(doi: str):
    ...
    try:
        r = requests.get(CROSSREF_API + quote(doi, safe=""), timeout=25,
                         headers={"User-Agent": "CardioUpdate/5.0"})
        if not r.ok:
            return None
        msg = (r.json() or {}).get("message") or {}
        return {
            "title": clean_markup((msg.get("title") or [""])[0]),
            "journal": clean_markup((msg.get("container-title") or [""])[0]),
...
def find_related_evidence(main: dict):
    ...
    if len(keywords) < 2:
        return []
    q = f'({" OR ".join(keywords[:6])}) AND HAS_ABSTRACT:Y sort_cited:y'
    pool = epmc_search(q, 50)
    mk = candidate_key(main)
    ranked, seen = [], set()
    for item in pool:
        k = candidate_key(item)
        if not k or k == mk or k in seen:
            continue
        seen.add(k)
    ...
    for _, item in ranked[:MAX_RELATED]:
        ...
        year = (item.get("firstPublicationDate") or "")[:4]
        if verified:
        ...
        related.append({
            ...
            "doi": doi, "pmid": item.get("pmid", ""),
            "type": type_text(item), "url": original_url(item),
            "abstract_context_es": translate_es(" ".join(re.split(r"(?<=[.!?])\s+", abstract)[:2])) if abstract else "",
...
def build_context_analysis(area: str, summary: str, related: list[dict]) -> str:
    if not related:
        return "Trabajo incorporado a la edición semanal; contextualización bibliográfica adicional pendiente."
    refs = "; ".join(f"{r.get('title','')} ({r.get('journal','')}, {r.get('year','')})" for r in related[:3])
    return (f"Este trabajo se sitúa dentro de {area.lower()}. Hallazgo principal según el abstract: {summary} "
            f"CardioUpdate recuperó {len(related)} fuentes relacionadas. Referencias prioritarias: {refs}. "
            "La síntesis automática no sustituye la lectura crítica del artículo completo ni una decisión clínica individual.")
...
def build_study(x: dict, sc: int):
    ...
    ptypes = type_text(x)
    typ = "Ensayo clínico" if re.search(r"trial|randomized|randomised", ptypes, re.I) else (
        "Revisión / meta-análisis" if re.search(r"review|meta", ptypes, re.I) else "Artículo científico")
    ...
    study = {
        ...
        "level": level(x),
        "type": typ,
        "journal": x.get("journalTitle") or (x.get("journalInfo") or {}).get("journal", {}).get("title", ""),
        "date": x.get("firstPublicationDate") or (x.get("journalInfo") or {}).get("printPublicationDate", ""),
        "summary": summary,
        "why": f"Seleccionado para la edición semanal por relevancia clínica, diseño y calidad de fuente en {area.lower()}.",
    ...
    if ai_analysis:
        study["ai_analysis"] = ai_analysis
        study["analysis_es"] = "\n\n".join([
            ai_analysis.get("main_finding", ""), ai_analysis.get("magnitude_and_results", ""),
            ai_analysis.get("prior_evidence", ""), ai_analysis.get("novelty", ""),
            ai_analysis.get("clinical_implications", ""), ai_analysis.get("uncertainties", "")
        ]).strip()
        study["analysis_mode"] = "ai_grounded_verified_sources"
        study["analysis_status"] = ai_analysis.get("analysis_status", "AUTO")
    else:
        study["analysis_status"] = "AUTO · Contexto bibliográfico"
    return study


def main():
    today = date.today()
    existing = load_json(DB, [])
    pool = load_json(CANDIDATES, [])
    recent = fetch_recent()
    pool = merge_candidate_pool(pool, recent)
    save_json(CANDIDATES, pool)

    publish = FORCE_PUBLISH or today.weekday() == 4
    if not publish:
        print(f"CardioUpdate: {len(pool)} candidatos acumulados. Edición publicada sin cambios hasta el viernes.")
        return

    start, end = edition_window(today)
    selected = select_weekly(pool, start, end)
    existing_map = {candidate_key(x): x for x in existing if candidate_key(x)}
    issue = []
    for x in selected:
        k = candidate_key(x)
        ...
        if old and old.get("review_status") != "Revisión pendiente":
            ...
            issue.append(build_study(x, score(x)))

    issue.sort(key=lambda s: ((s.get("auto_score") or 0), s.get("date") or ""), reverse=True)
    save_json(DB, issue)

    meta = load_json(META, {})
    meta.update({
        "updated_at": datetime.now(timezone.utc).isoformat(),
        "edition_start": start.isoformat(),
        "edition_end": end.isoformat(),
        "total_studies": len(issue),
        "candidate_pool": len(pool),
        "publication_mode": "weekly_friday",
        "source": "Europe PMC + Crossref",
        "related_evidence_enabled": True,
        "related_evidence_max_sources": MAX_RELATED,
    })
    save_json(META, meta)
